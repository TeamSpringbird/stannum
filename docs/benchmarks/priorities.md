...
|---|---|---|
| 1 | Attribute and bound construction memory | The 512 MiB build OOM is retained; four 2 GiB builds use roughly 1.15–1.20 GiB anonymous memory. Follow the [ranked hypotheses and controlled probes](build-memory-investigation.md). Measure accumulation versus encoding/merge allocations before changing defaults; preserve query and sustained-write controls. |
| 2 | Profile memory-sensitive queries and scale the working set | [Alternating 500k-document repeats](repeated-memory.md) show 7–8% less throughput and about 13% higher p95 at 512 MiB. Phrase p95 is most affected; long common-word AND/phrase forms are specific profiling candidates. The one-million probe passes at 352 QPS / 10.84 ms p95 with few guest reads. Use longer targeted traces, plans and work counters, then more readers and a repeated scale matrix. Guest reads are not physical-storage measurements. |
