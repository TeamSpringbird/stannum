...
|---|---|---|
| 1 | Reduce remaining merge retention, then bound execution memory | [Output allocation reuse](merge-output-reuse.md) cuts the measured merge peak from 1,298 to 831 MiB and lets the million-row small-batch case complete at 2 GiB. Both versions still OOM on the default 500k build at 1.25 GiB. [Earlier publication-buffer release](release-published-segment.md) also preserves correctness but still OOMs at 1.25 GiB. The [output-spill experiment](spill-merge-output.md) is the next bounded step; keep it disabled pending failure-path and memory evidence. Bound retained inputs and metadata next, accounting for allocator retention, directory limits and read amplification. Keep the existing batch default. |
...
| 3 | Close the query-shape coverage gaps | Use the [documentation audit](../research/tin-search-documentation.md) to add a small boundary oracle for max-score semantics, scoring arguments and highlighting. Extend performance coverage for fuzzy/regex/proximity, deeply nested Boolean trees, large K/OFFSET and secondary sorts, partitioned ranked queries, and cross-session snapshot/locking scenarios. Keep the routine Lead oracle within its roughly 15-minute budget; use larger same-engine checks for scale. |
