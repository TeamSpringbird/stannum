...
|---|---|---|
| 1 | Reduce remaining merge retention, then bound execution memory | [Output allocation reuse](merge-output-reuse.md) cuts the measured merge peak from 1,298 to 831 MiB and lets the million-row small-batch case complete at 2 GiB. Both versions still OOM on the default 500k build at 1.25 GiB. Next release the newly published blob before maintenance reads it back; then budget retained inputs/output and account for allocator retention, directory limits and read amplification. Keep the existing batch default. |
