...
Ranking reads the index too. `stannum.score`, `stannum.full_score` and `stannum.max_score`
...
* **Build.** `ambuild` tokenizes each heap tuple into an in-memory segment
  builder and writes a segment every `stannum.build_segment_docs` documents
  (default 32,768). The write buffer starts empty.
* **Insert.** One forward record is appended to the buffer under an exclusive
  lock on the meta page. When the buffer would exceed `stannum.write_buffer_docs`
...
* **Merge.** When the directory would exceed `stannum.max_segments` (default and
...
* Ranked queries score every candidate before selecting the top k: about
  ...
  end to end on that query; Stannum is at 4.3 ms. Block-level score bounds that
...
Same protocol, one run, after `stannum.score` moved to the index:
...
Two planner hooks add paths when `stannum.enable_custom_scan` is on (the default)
...
* **Stannum Text Search Scan** replaces the relation scan. It compiles the query
  ...
  rows with the original clause. When the query orders by `stannum.score`,
  `stannum.full_score` or `stannum.max_score` bound to the same index, the path claims
...
* **Stannum Count** replaces `SELECT count(*)` when the `==>` clause is the only
...
Stannum's from a local machine, so treat the comparison as coarse; the point is
