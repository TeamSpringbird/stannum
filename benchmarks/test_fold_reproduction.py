...
class StartupCleanupTests(unittest.TestCase):
    def check_startup_timeout(self, stop_fails):
        with tempfile.TemporaryDirectory() as directory:
            ...
            argv = ['fold_reproduction.py', '--codec', str(root / 'codec'),
                    '--combined', str(root / 'combined'), '--installed-library',
                    str(library), '--output', str(root / 'output')]
            with (patch('sys.argv', argv),
                  patch.object(fold_reproduction.tempfile, 'mkdtemp', return_value=str(cluster)),
                  patch.object(fold_reproduction.subprocess, 'run', side_effect=run),
                  contextlib.redirect_stdout(io.StringIO())):
                with self.assertRaises(subprocess.CalledProcessError):
                    fold_reproduction.main()
