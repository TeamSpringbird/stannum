...
def load(connect,queries,expected,clients,seconds,seed,folder,warm=False):
    ...
    def worker(number):
        ...
        try:
            with connect() as conn:
                ...
                if warm:
                    # Untimed: each backend touches every query once, in both arms.
                    for q in order:conn.execute('SELECT count(*) FROM documents WHERE body ==> %s',(q['engines']['tin']['disjunction'],)).fetchone()
                ...
                before=cpu();ready.wait(timeout=1800);go.wait(timeout=1800)
                cursor=0
    ...
    with ThreadPoolExecutor(max_workers=clients) as pool:
        ...
        ready.wait(timeout=1800);clock['client_cpu_start']=time.process_time();clock['start']=time.perf_counter();clock['deadline']=clock['start']+seconds;go.set()
...
def main():
    ...
    p.add_argument('--candidate-sql',help='Session SQL applied only to the candidate arm, such as an experimental setting');p.add_argument('--control-sql',help='Session SQL applied only to the control arm');p.add_argument('--client-warmup',action='store_true',help='Each client runs every query once before the timed window, in both arms')
                    ...
                    for clients in (a.clients if r%2==0 else list(reversed(a.clients))):
                        for variant in order:
                            ...
                            def configured():
                                ...
                                if a.candidate_sql and variant=='candidate':conn.execute(a.candidate_sql)
                                if a.control_sql and variant=='control':conn.execute(a.control_sql)
                            ...
                            summary=load(configured,queries,expected,clients,a.seconds,11000+r,folder,a.client_warmup)
