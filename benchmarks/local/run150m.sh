...
python3 benchmarks/tin.py --driver "$D" run ${STANNUM_SOURCE:+--source-manifest "$STANNUM_SOURCE"} --published-corpus stackexchange --dataset "$DS" --rows 150000000 \
  --validation-rows 1000 --validation-queries 10 --ranked-validation-queries 2 --engines stannum --workload topk --style $STYLE --updates $UPDATES \
  --warmup 10 --seconds $SECONDS_ --clients 8 --cpus 8 --memory 32g --build-memory 64g --shared-buffers 24GB --maintenance-work-mem 24GB \
  --setup-timeout-seconds 14400 --load-database ${STANNUM_MOCK:-/tmp/stannum-ordinal-poc/local150m}/db --image $IMG --output $R > $R.log 2>&1
