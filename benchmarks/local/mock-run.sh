...
# Counters reset when the harness restarts the container; keep the last monotonic stretch.
cut=0
for i in range(1,len(meas)):
    if int(meas[i]['counters']['cpu.stat'].split()[1]) < int(meas[i-1]['counters']['cpu.stat'].split()[1]): cut=i
meas=meas[cut:]
def io(r):
    t=0
